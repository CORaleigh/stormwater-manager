...
export class ApportionmentDialogComponent implements OnInit, OnDestroy {
  ...
  @Input() mode: string = "";
  @Input() apportionment: Apportionment | null = null;
  ...
  submitted(apportionment: Apportionment | null) {
    let updates = [];

    this.apportionment = apportionment;
    if (!this.apportionment || !this.account) return;
    this.apportionment.Impervious = Math.round(
      this.account.BillableImpervious ?? 0 * this.apportionment.PercentApportioned,
    );
    ...
    if (this.account.ApportionmentCode === "EQUAL") {
      this.apportionments.forEach((apportionment) => {
        ...
        apportionment.PercentApportioned =
          100 / this.account.ApportionmentUnits / 100;
        apportionment.Impervious = Math.round(
          this.account.BillableImpervious ?? 0 * apportionment.PercentApportioned,
        );
        //apportionment.Sfeu = this.account.Sfeu * apportionment.PercentApportioned;
    ...
    this.stormwater.applyEdits(5, adds, updates, []).subscribe((result) => {
      ...
      if (!apportionment || !this.apportionment || !this.account) return;
  ...
  apportionmentDeleted(apportionment: Apportionment) {
    this.stormwater
      .applyEdits(5, undefined, undefined, [apportionment.OBJECTID])
      .subscribe((result) => {
