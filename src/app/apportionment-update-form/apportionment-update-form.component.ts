...
export class ApportionmentUpdateFormComponent {
  ...
  private _apportionment: Apportionment | null = null;
  ...
  set apportionment(apportionment: Apportionment) {
    ...
    if (apportionment && this.account) {
      this.form
        .get("PercentApportioned")
        ?.setValue(this._apportionment.PercentApportioned * 100);
      ...
      let account = this.stormwater.account.getValue();
      this.setRemainingPercent(this.account, apportionment);
    }
  }
  @Input("account")
  ...
  getBilling(premiseId: string, type: string) {
  ...
  setRemainingPercent(account: Account, apportionment: Apportionment | null) {
    if (account.ApportionmentCode === "WEIGHTED") {
      let apportionments: Apportionment[] | null =
        this.stormwater.apportionments.getValue();
      let remaining = 100;
      ...
      this.form
        .get("PercentApportioned")
        ?.setValidators(Validators.max(remaining));
    }
  }
  ...
  checkInvalid() {
    return (
      ...
      this.form.get("PercentApportioned")?.invalid
    );
  }
  onSubmit() {
    if (this.mode === "add") {
      ...
      this._apportionment.ExpirationDate =
        moment().add(10, "years").unix() * 1000;
      this._apportionment.PremiseId = this.ccbAccount.premiseId.toString();
    }
    if (
      this._apportionment &&
      this._account &&
      this._account.ApportionmentCode === "WEIGHTED"
    ) {
      this._apportionment.PercentApportioned =
        this.form.get("PercentApportioned")?.value / 100;
    } else if (
      this._apportionment &&
      this._account?.ApportionmentCode === "EQUAL"
    ) {
      this._apportionment.PercentApportioned =
        100 / this._account.ApportionmentUnits / 100;
    }
    if (this._apportionment) {
