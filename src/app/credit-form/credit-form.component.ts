...
export class CreditFormComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.credit && this.form) {
      ...
      this.form
        ...
        ?.setValue(this.credit.ControlledSurface);
      this.form
        .get("NpdesPercentage")
        ?.setValue(this.credit.NpdesPercentage * 100);
      this.form
        .get("UpstreamPercentage")
        ?.setValue(this.credit.UpstreamPercentage);
  ...
  onSubmit() {
    if (this.credit) {
      ...
      this.credit.NpdesPercentage =
        this.form.get("NpdesPercentage")?.value / 100;
      this.credit.UpstreamPercentage =
        this.form.get("UpstreamPercentage")?.value;
