...
export class MapComponent implements OnInit, OnDestroy {
  ...
  private _parcelView: esri.FeatureLayerView | null = null;
  private _highlight: any;
  private _parcelGraphics: esri.GraphicsLayer | null = null;
  private _selectedParcel: esri.Graphic | null = null;
  private _lastAccountId: number | null = null;
  private _parcels: esri.FeatureLayer | null = null;
  streetNameSubscription: Subscription | null = null;
  accountSearchSubscription: Subscription | null = null;
  accountSubscription: Subscription | null = null;
  @Input("account") account: Account | null = null;
  constructor(
    public stormwater: StormwaterService,
    private billing: BillingService,
    private route: ActivatedRoute,
    private router: Router,
    private location: Location,
    private lods: LodsService,
  ) {}
  async authenicate() {
    try {
  ...
  checkSignInStatus() {
    if (!this._esriId || !this._info) return;
    this._esriId
      .checkSignInStatus(this._info.portalUrl + "/sharing")
      .then((event) => {
  ...
  async initializeMap() {
    try {
      ...
      this.stormwater.mapview.when((mapView: MapView) => {
        mapView?.ui?.container?.classList?.remove("calcite-mode-light");
        ...
        this.stormwater.mapview.on("hold", (e: any) => {
          ...
          if (event.action.id === "assign-csaid") {
            if (mapView.popup.selectedFeature.attributes.CSAID) {
              ...
              if (newCsa != oldCsa) {
                ...
                this.stormwater
                  .applyEdits(2, undefined, [new Feature(account)])
                  .subscribe((result) => {
  ...
  configMapWidgets(mapView: esri.MapView) {
    try {
      const list: esri.LayerList = new LayerList({
        view: mapView,
        container: document.createElement("div"),
      });
      const listExpand: esri.Expand = new Expand({
        expandIcon: "layers",
        view: mapView,
        content: list,
      });
      mapView.ui.add(listExpand, "top-right");
      ...
      const gallery: esri.BasemapGallery = new BasemapGallery({
        view: mapView,
        container: document.createElement("div"),
      });
      const galleryExpand: esri.Expand = new Expand({
        expandIcon: "basemap",
        view: mapView,
        content: gallery,
      });
      ...
      listExpand.watch("expanded", (expanded) => {
        if (expanded) {
          legendExpand.collapse();
          galleryExpand.collapse();
        }
      });
      ...
      galleryExpand.watch("expanded", (expanded) => {
        if (expanded) {
          legendExpand.collapse();
          listExpand.collapse();
        }
  ...
  configBasemapGallery(mapView: esri.MapView) {
    try {
    } catch (error) {
      console.log("We have an error: " + error);
    }
  ...
  getSource(
    layer: esri.Layer,
    LayerSearchSource: any,
    displayField: string,
    name: string,
    where: string,
    placeholder: string,
  ): esri.LayerSearchSource {
  ...
  async setupSearch(mapView: esri.MapView) {
    try {
      ...
      search.sources.push(
        new LayerSearchSource({
          layer: new FeatureLayer({
            url: `${this._serverUrl}/arcgis/rest/services/Stormwater/Stormwater_Management/FeatureServer/0`,
          }),
      ...
      search.goToOverride = (view: esri.MapView | esri.SceneView, params: any) => {
        if (
          params.target.target.layer.title.indexOf("Stormwater Management - ") >
            -1 &&
          params.target.target.layer.isTable
        ) {
          this.account = params.target.target.attributes as Account;
          this.stormwater.accounts.next([this.account]);

          this.stormwater.account.next(
            params.target.target.attributes as Account,
          );
          this.clearResultsList();

          this.getParcel(
            this.stormwater.parcels.url,
            esriRequest,
            params.target.target.attributes.OBJECTID,
            view as esri.MapView,
          );
          this._search?.clear();
        } else {
          this.getParcelByObjectId(
            params.target.target.attributes.OBJECTID,
            mapView,
          );
      ...
      search.on("select-result", (event) => {
        if (
          event.source.name != "Address Point" &&
          !event.source.layer.isTable
        ) {
          this.getAccount(event.result.feature);

          this.stormwater.parcel.next(
            event.result.feature.attributes as Parcel,
          );
          this.clearResultsList();
          this.account = null;
          this.stormwater.accountListSelected.next(null);
  ...
  queryRelatedTables(
    ...
    let promise = new Promise<any>((resolve, reject) => {
      ...
      query
        .executeRelationshipQuery(this.stormwater.parcels.url + "/2", {
          objectIds: [objectId],
  ...
  queryTables(
    ...
    esriRequest(url + "/2?f=json", { responseType: "json" }).then(
      (response: any) => {
        response.data.relationships.forEach((relationship: any) => {
          if (relationship.role === "esriRelRoleOrigin") {
            this.queryRelatedTables(url, relationship, query, objectId).then(
              (result) => {
                let attributes: any[] = [];
                if (result[objectId]) {
          ...
          if (relationship.role === "esriRelRoleDestination") {
            ...
            query
              ...
              .then((result) => {
                if (result[objectId]) {
                  this.getParcelByObjectId(
                    result[objectId].features[0].attributes.OBJECTID,
                    mapView,
                  );
                } else {
  ...
  getByAccountId(
    ...
    if (this.stormwater.parcels) {
      ...
      query
        .executeQueryJSON(this.stormwater.parcels.url + "/2", {
          where: field + " in (" + id.toString() + ")",
          outFields: ["*"],
          returnGeometry: false,
        })
        .then((result) => {
          if (result.features) {
            if (result.features.length < 2) {
              ...
              if (relationship) {
                this.queryParcelsRelatedToAccounts(
                  this.stormwater.parcels.url + "/2",
                  relationship.id,
                  query,
                  oids,
                ).then((parcelResult) => {
                  ...
                  result.features.forEach((f) => {
                    if (parcelResult[f.attributes.OBJECTID]) {
                      let feature =
                        parcelResult[f.attributes.OBJECTID].features[0];
                      data.push({
                        SiteAddress: feature.attributes.SiteAddress,
                        RealEstateId: feature.attributes.RealEstateId,
                        AccountId: f.attributes.AccountId,
                        Status: f.attributes.Status,
                        TotalImpervious: f.attributes.TotalImpervious,
                        ApportionmentUnits: f.attributes.ApportionmentUnits,
  ...
  zoomToParcels(features: __esri.Graphic[], mapView: __esri.MapView) {
    let geoms: any[] = [];
  ...
  ngOnInit() {
    ...
    this.stormwater.credentials.subscribe((credentials) => {
      if (credentials) {
        this.stormwater
          .getDataElements(credentials.token)
          .subscribe((result) => {
    ...
    this.streetNameSubscription = this.stormwater.streetName.subscribe(
      (streetName) => {
        if (streetName && this.stormwater.parcels) {
          this.stormwater.parcels
            .queryFeatures({
              where: "FullStreetName = '" + streetName + "'",
              returnGeometry: true,
              outFields: ["*"],
    ...
    this.accountSearchSubscription = this.stormwater.accountSearch.subscribe(
      (selection) => {
        if (selection) {
          let field: string = "";
          let value: any = null;
            ...
            if (result) {
              ...
              features.forEach((f) => {
                ...
                data.push({
                  ...
                  updated: f.attributes.UPDATE_DATE,
                });
              });
