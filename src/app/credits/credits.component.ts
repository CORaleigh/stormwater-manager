...
export class CreditsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.creditsSubscription = this.stormwater.credits.subscribe(
  ...
  delete() {
    let confirm = this.dialog.open(DialogComponent, {
      data: {
        title: "Confirm",
        message:
          "This will delete the credit for this account, would you like to continue?",
        yesno: true,
      },
    });
    confirm.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        let deletes = [this.credits[0].OBJECTID as number];
        this.stormwater
          .applyEdits(4, undefined, undefined, deletes)
          .subscribe((result) => {
            this.stormwater.credits.next([]);
            const account = this.stormwater.account.getValue();
            if (account) {
              account.CreditedImpervious = 0;
              account.BillableImpervious = account.TotalImpervious ?? 0 - 0;
            }

            this.stormwater
              .applyEdits(2, undefined, [
                {
                  attributes: this.stormwater.account.getValue(),
                } as __esri.Graphic,
              ])
              .subscribe((result) => {
                this.stormwater.accountListSelected.next(
                  this.stormwater.account.getValue(),
                );
              });
          });
      }
  ...
  update() {
    ...
    let ref = this.dialog.open(DialogComponent, {
      data: { title: "Update Credit", credit: this.credit },
    });
    ref.afterClosed().subscribe((data: Credit) => {
      if (data) {
        ...
        this.stormwater
          .applyEdits(4, adds, updates, undefined)
          .subscribe((result) => {
