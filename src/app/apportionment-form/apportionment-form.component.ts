...
export class ApportionmentFormComponent implements OnInit, OnDestroy {
  ...
  codeChanged(event: any) {
    let account: Account | null = this.stormwater.account.getValue();
    if (!account) return;
    ...
    if (account.ApportionmentCode === "NA") {
      let ref = this.dialog.open(DialogComponent, {
        data: {
          title: "Confirm",
          message:
            "Setting apportionment code to NA will delete all apportionments associated with this account.  Would you like to continue?",
          yesno: false,
        },
      });
      ref.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          ...
          this.stormwater
            .applyEdits(5, undefined, undefined, deletes)
            .subscribe((result) => {
    ...
    this.stormwater
      .applyEdits(2, undefined, [new Feature(account, null)])
      .subscribe((result) => {
        this.updateApportionments(account);
      });
  }
  unitsChanged(event: any) {
    let account = this.stormwater.account.getValue();
    const units = this.apptForm.get("units");
    if (account && units && units.value != 0) {
      if (units.value >= this.apportionments.length) {
        account.ApportionmentUnits = units.value;
        this.stormwater
          .applyEdits(2, undefined, [new Feature(account, null)])
          .subscribe((result) => {
            console.log(result);
        ...
        let ref = this.dialog.open(DialogComponent, {
          data: {
            title: "Confirm",
            message:
              "The value entered is less than the number of existing apportionments.",
            yesno: false,
          },
        });
        units.setValue(this.apportionments.length);
      }
    }
  }
  checkCode() {
    const code: any = this.apptForm.get("code");
    if (code.value === "NA" || !code.value) {
      return true;
    } else {
      return false;
    }
  ...
  updateApportionments(account: Account) {
    if (account.ApportionmentCode === "EQUAL") {
      ...
      this.apportionments.forEach((apportionment) => {
        apportionment.PercentApportioned =
          100 / account.ApportionmentUnits / 100;
        //apportionment.Sfeu = apportionment.PercentApportioned * account.Sfeu;
        apportionment.Impervious = Math.round(
          apportionment.PercentApportioned * (account.BillableImpervious ?? 0),
        );
