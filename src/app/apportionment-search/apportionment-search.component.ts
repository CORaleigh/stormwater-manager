...
export class ApportionmentSearchComponent implements OnInit {
  ...
  getBilling(event: any, type: string) {
    if (this.count < this.types.length) {
      this.billing
        .getBillingInfo(event.option.value.premiseId, type)
        .subscribe((data) => {
          console.log(data);
          if (data.results) {
  ...
  inputChanged(event: any) {
    ...
    this.addressChanges = this.billing
      .searchCcbAccounts("address", event.target.value)
      .subscribe((result) => {
        if (result.results) {
          result.results.forEach((r: any) => {
            //@ts-ignore
            this.searchGroups[0].values.push({
              value: r.address,
        ...
        this.accountChanges = this.billing
          .searchCcbAccounts("account", event.target.value)
          .subscribe((result) => {
            if (result.results) {
              result.results.forEach((r: any) => {
                if (r.premiseId.length) {
                  //@ts-ignore
                  this.searchGroups[1].values.push({
                    value: r.accountId,
                    premiseId: r.premiseId,
                    address: r.address,
                  });
            ...
            this.premiseChanges = this.billing
              .searchCcbAccounts("premise", event.target.value)
              .subscribe((result) => {
                if (result.results) {
                  result.results.forEach((r: any) => {
                    //@ts-ignore
                    this.searchGroups[2].values.push({
                      value: r.premiseId,
                      premiseId: r.premiseId,
                      address: r.address,
                    });
